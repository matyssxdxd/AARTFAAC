...
:
  ...
  weights(boost::extents[channels][taps])
{
  generate_filter();
}
...
void FilterBank::interpolate(const double x[], const double y[], unsigned xlen, unsigned n, double result[])
{
  ...
  for (double interpolatedX = 0; interpolatedX <= 1.0; interpolatedX += 1.0 / (n - 1), index++) {
    ...
    if (nextX == 0) {
      std:: cerr << "ERROR in FilterBank::interpolate" << std::endl;
    }
...
#pragma omp critical (FFTW)
  {
    ...
    if (cinput == NULL || coutput == NULL) {
      throw Exception("cannot allocate buffers");
    }
...
#if 0
  std::clog << "final taps: " << std::endl;
  for(unsigned channel=0; channel<itsNrChannels; channel++) {
    std::clog << "channel: " << channel << "| ";
    for(unsigned tap=0; tap<itsNrTaps; tap++) {
      std::clog << " " << weights[channel][tap];
    }
    std::clog << std::endl;
  }
#endif
...
{
  if(itsVerbose) {
    std::clog << "using original static 256 channel FIR filter bank" << std::endl;
  }
...
{
  std::cout << (itsNegated ? "NEGATED" : "NORMAL(NOT NEGATED)") << std::endl;
  for (int tap = itsNrTaps - 1; tap >= 0; tap--) { // taps are stored in reverse!
    for (unsigned channel = 0; channel < itsNrChannels; channel++) {
      if (itsNegated && channel % 2 != 0) {
        std::cout << -weights[channel][tap] << std::endl; // odd channels are negated
      } else {
        std::cout << weights[channel][tap] << std::endl;
      }
    }
  }
}
