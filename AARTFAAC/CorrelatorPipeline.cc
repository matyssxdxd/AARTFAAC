...
volatile std::sig_atomic_t AARTFAAC_CorrelatorPipeline::signalCaught = false;


AARTFAAC_CorrelatorPipeline::AARTFAAC_CorrelatorPipeline(const AARTFAAC_Parset &ps)
:
...
void AARTFAAC_CorrelatorPipeline::doWork()
{
...
bool AARTFAAC_CorrelatorPipeline::getWork(unsigned preferredNode, TimeStamp &time, unsigned &subband)
{
  if (signalCaught)
    return false;
...
void AARTFAAC_CorrelatorPipeline::logProgress(const TimeStamp &time) const
{
...
void AARTFAAC_CorrelatorPipeline::startReadTransaction(const TimeStamp &time)
{
...
void AARTFAAC_CorrelatorPipeline::endReadTransaction(const TimeStamp &time)
{
  std::lock_guard<std::mutex> lock(currentTimesMutex);
...
void AARTFAAC_CorrelatorPipeline::caughtSignal()
{
  signalCaught = true;
